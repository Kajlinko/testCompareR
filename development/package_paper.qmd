...
```{r}
nodes <- create_node_df(nodes = 1:7, type = "number",
                      label = c("Results", "cont\nContingency Tables", 
                                "prev\nPrevalence", "acc\nDiagnostic Accuracies",
                                "pv\nPredictive Values", "lr\nLikelihood Ratios",
                                "other\nOther Values"), n = 1)

edges <- create_edge_df(from = c(1,1,1,1,1,1), to = c(2,3,4,5,6,7), rel = "leading to")
