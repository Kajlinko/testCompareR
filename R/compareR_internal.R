...
#' @title check.df
#'
#' @description Takes a data frame with two or three columns containing the results of binary diagnostic test(s) and a gold standard test and checks it conforms to standards required for the compareR function.
#'
#' @param df A data frame with two or three columns. Users may code positive results as strings (positive, pos, p, yes, y, +, 1), numeric (1) or factor (1). Users may code positive results as strings (negative, neg, no, n, -, 0, 2), numeric (0, 2) or factor (0, 2). Case and white space insensitive.
#'
#' @return A Boolean value indicating whether all entries in the data frame meet the requirements described in arguments.
#'
#' @keywords internal
#'
#' @examples
...
#'   column2 = c(" NEG ", " yes ", " negative ", " Y ", "-", " 0 ")
#' )
#'
#' # check the dataframe
...
#' @title recoder
#'
#' @description Takes a data frame with two or three columns containing the results of binary diagnostic test(s) and a gold standard test and re-codes to an acceptable format for the compareR function.
#'
#' @param df A dataframe with two or three columns. Users may code positive results as strings (positive, pos, p, yes, y, +, 1), numeric (1) or factor (1). Users may code positive results as strings (negative, neg, no, n, -, 0, 2), numeric (0, 2) or factor (0, 2). Case and white space insensitive.
#'
#' @return A data frame with the same number of columns as df re-coded to integers of 0 (negative) and 1 (positive).
#'
#' @keywords internal
#'
#' @examples
...
#' @title values.1test
#'
#' @description Creates a list object containing important values for future calculations.
#'
#' @param df A dataframe with 2 columns (test1, gold), populated with zeros and ones.
#'
...
#' @description Creates a list object containing important values for future calculations.
#'
#' @param df A dataframe with 3 columns (test1, test2, gold), populated with zeros and ones.
#'
#' @return A list object containing important values for future calculations.
#'
#' @keywords internal
...
#' @title disp.cont
#'
#' @description Creates and displays a contingency table of positive and negative test results and disease status.
#'
#' @param vals A list object containing all values required for calculations with class `'vals.1test'` or `'vals.2test'`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param margins A Boolean value indicating whether the outputted matrix should have margins containing summed totals of rows and columns.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return Either a matrix or list of matrices displaying counts of positive and negative tests by true disease status.
#'
#' @keywords internal
#'
#' @examples
...
#' @title yu.int
#'
#' @description Calculates the Yu et al. confidence interval for binomial proportions.
#'
...
yu.int <- function(n, z, est) {
  ...
  if(!is.na(est) && (est < 0 | est > 1)) stop("Point estimate must be between 0 and 1, inclusive.")

  # FUNCTION

  lower <- 0.5 + ((n + z^4 / 53) / (n + z^2)) * (est - 0.5) - (z / (n + z^2)) * sqrt(n * est * (1 - est) + z^2 / 4)
  upper <- 0.5 + ((n + z^4 / 53) / (n + z^2)) * (est - 0.5) + (z / (n + z^2)) * sqrt(n * est * (1 - est) + z^2 / 4)
  if(is.na(est)) {return(list(lower = 0, upper = 1))}
  else if(lower >= 0 && upper <= 1) {return(list(lower = lower, upper = upper))}
  else if(lower >= 0 && upper > 1) {return(list(lower = lower, upper = 1))}
  else if(lower < 0 && upper <= 1) {return(list(lower = 0, upper = upper))}
  else {stop("Confidence interval includes values less than zero and greater than one. Check input and consider reporting a bug.")}
...
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object of class `'prev'` containing a point estimate, plus lower and upper confidence bounds for disease prevalence.
#'
#' @keywords internal
#'
#' @examples
...
conf.prev <- function(vals, alpha = 0.05, ...) {
  ...
  if(alpha < 0 | alpha > 1) stop("Alpha must be a value between 0 and 1, inclusive.")
  if(is.na(vals$prev)) stop("Prevalence should not be NA/NaN. Check data and consider reporting a bug.")
...
#' @title conf.acc
#'
#' @description Calculates confidence intervals for accuracies (sensitivity and specificity) for one or two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param alpha An alpha value. Defaults to 0.05.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object of class `'conf.1t'` or `'conf.2t'` containing point estimates, plus lower and upper confidence bounds for diagnostic accuracies.
#'
#' @keywords internal
#'
#' @examples
...
#' @title conf.pv
#'
#' @description Calculates confidence intervals for positive and negative predictive values for one or two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param alpha An alpha value. Defaults to 0.05.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object of class `'conf.1t'` or `'conf.2t'` containing point estimates, plus lower and upper confidence bounds for predictive values.
#'
#' @keywords internal
#'
#' @examples
...
#' @param z Z-score of normal distribution at a defined alpha level.
#'
#' @return A list object of class `'conf.1t'` or `'conf.2t'` containing lower and upper confidence bounds for positive likelihood ratio.
#'
#' @keywords internal
#'
#' @examples
...
#' @param z Z-score of normal distribution at a defined alpha level.
#'
#' @return A list object of class `'conf.1t'` or `'conf.2t'` containing lower and upper confidence bounds for negative likelihood ratio.
#'
#' @keywords internal
#'
#' @examples
...
#' @title conf.lr
#'
#' @description Calculates confidence intervals for positive and negative likelihood ratios for one or two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param alpha An alpha value. Defaults to 0.05.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object of class `'conf.1t'` or `'conf.2t'` containing point estimates, plus lower and upper confidence bounds for likelihood ratios.
#'
#' @keywords internal
#'
#' @examples
...
#' @title output.acc
#'
#' @description Performs statistical inference tests on the diagnostic accuracies of two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object containing test statistics and p-values for global and individual hypothesis tests of diagnostic accuracies.
#'
#' @keywords internal
#'
#' @examples
...
#' @title output.pv
#'
#' @description Performs statistical inference tests on the predictive values of two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object containing test statistics and p-values for global and individual hypothesis tests of predictive values.
#'
#' @keywords internal
#'
#' @examples
...
#' @title output.lr
#'
#' @description Performs statistical inference tests on the likelihood ratios of two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param ... Rarely needs to be used. Allows additional arguments to be passed to internal functions.
#'
#' @return A list object containing test statistics and p-values for global and individual hypothesis tests of likelihood ratioss.
#'
#' @keywords internal
#'
#' @examples
...
#' @title matrixify
#'
#' @description Produces a matrix containing point estimates, standard errors and upper and lower confidence bounds for related outputs (eg. sensitivity and specificity) of one or two binary diagnostic tests.
#'
#' @param vals A list object containing all values required for calculations with class `vals.1test` or `vals.2test`. These objects are output by the `values.1test()` and `values.2test()` function.
#' @param rows A vector of length two giving the names of the related tests (eg. sensitivity and specificity).
#' @param test.names A vector of length two giving the names of the two different binary diagnostic tests. This argument is not relevant when testing a single binary diagnostic test.
#' @param dp Number of decimal places of output. Defaults to 1.
#'
#' @return Either a matrix or list of matrices displaying point estimates, standard errors and upper and lower confidence bounds for related outputs (eg. sensitivity and specificity) of one or two binary diagnostic tests.
#'
#' @keywords internal
#'
#' @examples
...
matrixify <- function(vals, rows = c("Row 1", "Row 2"), test.names = c("Test 1", "Test 2"), dp = 3) {

  if(inherits(vals, "conf.1t")) {

    mat <- matrix(rep(0,8), nrow = 2, ncol = 4,
