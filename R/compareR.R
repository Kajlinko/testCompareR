...
#' @title compareR
#'
#' @description Calculates descriptive statistics and performs statistical inference on two binary diagnostic tests in a single function call. Handles multiple comparisons using methods in `p.adjust()`.
#'
#' @param df A data frame or matrix with 3 columns (test1, test2, gold). Flexible coding of positive and negative results permitted.
#' @param alpha An alpha value. Defaults to 0.05.
#' @param margins A Boolean value indicating whether the contingency tables should have margins containing summed totals of rows and columns.
#' @param multi_corr Method for multiple comparisons. Uses `p.adjust.methods`.
#' @param cc A Boolean value indicating whether McNemar's test should be applied with continuity correction.
#' @param dp Number of decimal places of output in summary tables. Defaults to 1.
#' @param sesp A Boolean value indicating whether output should include sensitivity and specificity.
#' @param ppvnpv A Boolean value indicating whether output should include positive and negative predictive values.
#' @param plrnlr A Boolean value indicating whether output should include positive and negative likelihood ratios.
...
#'
#' @return A list objective summarising all calculated descriptive and inferential statistics.
#'
#' @details
#' Confidence intervals for prevalence, diagnostic accuracies and predictive values are calculated using the interval for binomial proportions described by Yu et al. (2014).
#' Confidence intervals for likelihood ratios are calculated using the methods recommended by Martín-Andrés and Álvarez-Hernández (2014).
#' Hypothesis testing for diagnostic accuracies uses different methods depending on disease prevalence and number of participants or samples as described by Roldán-Nofuentes and Sidaty-Regad (2019).
#' Global hypothesis testing for predictive values uses a method described by Roldán-Nofuentes et al. (2012), with subsequent individual tests (where indicated) performed using methods described by Kosinksi (2012).
#' The methods for hypothesis testing- for likelihood ratios are taken from Roldán-Nofuentes & Luna del Castillo (2007).
#'
#' An excellent summary of these methods is provided by Roldán-Nofuentes (2020) along with an open-source program (compbdt) licensed under GPL-2. This R package can be considered an extension of this work and is therefore distributed under the same license.
#' Please consider citing Roldán-Nofuentes (2020) when you are citing this package.
#'
#' @references
...
#' compareR(df, margins = TRUE, multi_corr = "bonf",
#'          test.names = c("pulm.exac","pseudomonas"))
#'
#' @export
#' @importFrom stats "p.adjust"

compareR <- function(df, alpha = 0.05, margins = FALSE, multi_corr = "holm", cc = TRUE, dp = 1,
  ...
  if(sesp == TRUE) {
    ...
    acc.mat <- matrixify(acc.est, rows = c("Sensitivity", "Specificity"), test.names = test.names, dp = dp+2)
  ...
  if(ppvnpv == TRUE) {
    ...
    pv.mat <- matrixify(pv.est, rows = c("PPV", "NPV"), test.names = test.names, dp = dp+2)
  ...
  if(plrnlr == TRUE) {
    ...
    lr.mat <- matrixify(lr.est, rows = c("PLR", "NLR"), test.names = test.names, dp = dp + 2)
  ...
  zeros <- 0
  counts <- c(vals$s11, vals$s10, vals$s01, vals$s00,
  ...
  equal <- FALSE
  if(vals$s10 == vals$s01 && vals$r10 == vals$r01) {equal <- TRUE}

  other <- list(alpha = alpha,
                equal = equal,
                zeros = zeros,
                Youden1 = Y1,
                Youden2 = Y2,
  ...
  if(sesp == T && ppvnpv == T && plrnlr == T) {
    out <- list(cont, prev, acc, pv, lr, other); names(out) <- c("cont", "prev", "acc", "pv", "lr", "other")}
  else if(sesp == T && ppvnpv == T && plrnlr == F) {
    out <- list(cont, prev, acc, pv, other); names(out) <- c("cont", "prev", "acc", "pv", "other")}
  else if(sesp == T && ppvnpv == F && plrnlr == T) {
    out <- list(cont, prev, acc, lr, other); names(out) <- c("cont", "prev", "acc", "lr", "other")}
  else if(sesp == F && ppvnpv == T && plrnlr == T) {
    out <- list(cont, prev, pv, lr, other); names(out) <- c("cont", "prev", "pv", "lr", "other")}
  else if(sesp == T && ppvnpv == F && plrnlr == F) {
    out <- list(cont, prev, acc, other); names(out) <- c("cont", "prev", "acc", "other")}
  else if(sesp == F && ppvnpv == T && plrnlr == F) {
    out <- list(cont, prev, pv, other); names(out) <- c("cont", "prev", "pv", "other")}
  else if(sesp == F && ppvnpv == F && plrnlr == T) {
    out <- list(cont, prev, lr, other); names(out) <- c("cont", "prev", "lr", "other")}
