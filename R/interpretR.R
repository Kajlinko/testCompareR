...
#' @title interpretR
#'
#' @description Provides a plain English readout of the results of the compareR function.
#'
#' @param result A list object with class 'compareR' output from the compareR function.
#'
#' @return A plain English summary of the findings produced by the compareR function.
#'
#' @examples
#' # simulate data
...
interpretR <- function(result) {
  ...
  if(!inherits(result, "compareR")) {
    stop("The interpretR() function can only interpret output from the compareR() function.")
  }

  ## warnings about data quality issues that may affect results

  if(result$other$equal == TRUE | result$other$zeros > 0 | result$other$Youden1 <= 0 | result$other$Youden1 <= 0) {
    cat("\nWARNING:\n")
  ...
  if(!is.null(result$acc)) {
    ...
    if(is.na(result$acc$glob.p.adj)) {
      ...
      if(result$acc$glob.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
    }
    if(is.na(result$acc$glob.p.adj) | result$acc$glob.p.adj < alpha) {
      ...
      if(!is.na(result$acc$sens.p.adj) && result$acc$sens.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
      ...
      if(!is.na(result$acc$spec.p.adj) && result$acc$spec.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
  ...
  if(!is.null(result$pv)) {
    ...
    if(is.na(result$pv$glob.p.adj)) {
      ...
      if(result$pv$glob.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
    }
    if(is.na(result$pv$glob.p.adj) | result$pv$glob.p.adj < alpha) {
      ...
      if(!is.na(result$pv$ppv.p.adj) && result$pv$ppv.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
      ...
      if(!is.na(result$pv$npv.p.adj) && result$pv$npv.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
  ...
  if(!is.null(result$lr)) {
    ...
    if(is.na(result$lr$glob.p.adj)) {
      ...
      if(result$lr$glob.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
    }
    if(is.na(result$lr$glob.p.adj) | result$lr$glob.p.adj < alpha) {
      ...
      if(!is.na(result$lr$plr.p.adj) && result$lr$plr.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
      ...
      if(!is.na(result$lr$nlr.p.adj) && result$lr$nlr.p.adj < alpha) {cat(" ***SIGNIFICANT***")}
